...
sourceDF = df_job_hiring_process
sourceDF.display()
...
# MAGIC %sql
# MAGIC 
# MAGIC SELECT COUNT(*) FROM jobposts_noblehire.job_hiring_process WHERE IsActive = True
# MAGIC -- DROP TABLE jobposts_noblehire.job_hiring_process

# COMMAND ----------

# DBTITLE 1,Create Delta Table Instance
deltaJobHiringProcess = DeltaTable.forPath(spark, "/mnt/adlslirkov/it-job-boards/Noblehire.io/delta/job_hiring_process")
...
if len(newColumns) > 0:
    for columnObject in newColumns:
        spark.sql("ALTER TABLE jobposts_noblehire.job_hiring_process ADD COLUMN ({} {})".format(columnObject[0], columnObject[1]))
...
deltaFinalPosts = DeltaTable.forPath(spark, "/mnt/adlslirkov/it-job-boards/Noblehire.io/delta/job_hiring_process")
