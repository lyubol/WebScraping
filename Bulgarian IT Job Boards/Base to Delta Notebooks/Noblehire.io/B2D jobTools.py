...
# MAGIC %sql
# MAGIC 
# MAGIC SELECT COUNT(*) FROM jobposts_noblehire.job_tools WHERE IsActive = True
...
if len(newColumns) > 0:
    for columnObject in newColumns:
        spark.sql("ALTER TABLE jobposts_noblehire.job_tools ADD COLUMN ({} {})".format(columnObject[0], columnObject[1]))
...
deltaFinalPosts = DeltaTable.forPath(spark, "/mnt/adlslirkov/it-job-boards/Noblehire.io/delta/job_tools")
