...
targetExceptSourceCount = df_awards_noblehire_enriched.where(col("IsActive") == True).select("AwardsId").exceptAll(sourceDF.select("AwardsId")).count()
targetEqualsSourceCount = df_awards_noblehire_enriched.where(col("IsActive") == True).count() == sourceDF.count()
